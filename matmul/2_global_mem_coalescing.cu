// Global memory coalescing: Sequential memory access by threads that are part of the sam ewarp can be grouped and executed as one.
// The idea is that we want threads that are in the same warp (neighboring indices) to access memory sequentially.

//Execution time: 20ms on RTX3090
...
{
    // Only change from naive matmul
    int idx = blockIdx.x * BLOCK_SIZE + (threadIdx.x/BLOCK_SIZE);
    int idy = blockIdx.y * BLOCK_SIZE + (threadIdx.x % BLOCK_SIZE);
